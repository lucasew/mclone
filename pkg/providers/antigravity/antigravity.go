	...
	go func() {
		...
		defer func() {
			if err := resp.Body.Close(); err != nil {
				monitor.ReportError(ctx, err, "action", "close_antigravity_stream")
			}
		}()
	...
	for attempt := 0; ; attempt++ {
		...
		for i := 0; i < len(codeAssistEndpoints); i++ {
			...
			if resp.StatusCode == http.StatusTooManyRequests || resp.StatusCode == http.StatusServiceUnavailable {
				...
				if err := resp.Body.Close(); err != nil {
					monitor.ReportError(ctx, err, "action", "close_antigravity_rate_limit")
				}
			...
			if resp.StatusCode >= 500 {
				respBody, _ := io.ReadAll(resp.Body)
				if err := resp.Body.Close(); err != nil {
					monitor.ReportError(ctx, err, "action", "close_antigravity_server_error")
				}
			...
			respBody, _ := io.ReadAll(resp.Body)
			if err := resp.Body.Close(); err != nil {
				monitor.ReportError(ctx, err, "action", "close_antigravity_error_status")
			}
...
func (p *Provider) onboardManagedProject(ctx context.Context, accessToken string) (string, error) {
	...
	defer func() {
		if err := resp.Body.Close(); err != nil {
			monitor.ReportError(ctx, err, "action", "close_antigravity_onboard")
		}
	}()
	respBody, _ := io.ReadAll(resp.Body)
	if resp.StatusCode != http.StatusOK {
	...
	for i := 0; i < 12; i++ {
		...
		if err := respOp.Body.Close(); err != nil {
			monitor.ReportError(ctx, err, "action", "close_antigravity_operation")
		}
...
func (p *Provider) refreshToken(refreshToken string) (*TokenData, error) {
	...
	defer func() {
		if err := resp.Body.Close(); err != nil {
			monitor.ReportError(context.Background(), err, "action", "close_antigravity_refresh_token")
		}
	}()
	if resp.StatusCode != http.StatusOK {
...
func (p *Provider) exchangeCode(code, verifier string) (*TokenData, error) {
	...
	defer func() {
		if err := resp.Body.Close(); err != nil {
			monitor.ReportError(context.Background(), err, "action", "close_antigravity_exchange_code")
		}
	}()
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(resp.Body)
