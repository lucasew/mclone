...
var serveCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		handleOpenAI := func(w http.ResponseWriter, r *http.Request) {
			...
			mcloneReq := remote.ChatRequest{Model: req.Model, Messages: req.Messages}
			respChan, err := p.Chat(r.Context(), mcloneReq)
			if err != nil {
		...
		handleAnthropic := func(w http.ResponseWriter, r *http.Request) {
			...
			if req.Stream {
				...
				for resp := range respChan {
					...
					// Anthropic stream format for text delta
		...
		fmt.Printf("Serving remote %q (OpenAI & Anthropic) on http://localhost:%d\n", remoteName, port)
